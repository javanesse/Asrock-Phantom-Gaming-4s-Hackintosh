/*
 * Patch Langkah 2
 * www.tonymacx86.com/threads/asus-100-series-and-later-custom-ssdt-for-xhci-usb-port-control.253981
 */
 ...
 *
 * Disassembly of SSDT-3-xh_cfsd4.aml, Thu Jun  6 20:59:04 2019
 *
 * Original Table Header:
 *     Signature        "SSDT"
 *     Length           0x00002950 (10576)
 *     Revision         0x02
 *     Checksum         0x7E
 *     OEM ID           "INTEL"
 ...
 */
DefinitionBlock ("SSDT-3", "SSDT", 2, "INTEL", "xh_cfsd4", 0x00000000)
{
    ...
    External (ATDV)    // (from opcode)
    External (BED2)    // (from opcode)
    External (BED3)    // (from opcode)
    External (BTBR)    // (from opcode)
    External (BTL2)    // (from opcode)
    External (BTLE)    // (from opcode)
    External (BTLL)    // (from opcode)
    External (BTSE)    // (from opcode)
    ...
    If (LGreaterEqual (NSSP, 0x03))
    {
        Scope (\_SB.PCI0.XHC.RHUB.SS03)
        {
            Method (_UPC, 0, NotSerialized)  // _UPC: USB Port Capabilities
            {
                //jv USB 3 belakang tengah dalam
                If (LEqual (S3TC, Zero))
                {
                    Return (GUPC (S3CN))
                }
